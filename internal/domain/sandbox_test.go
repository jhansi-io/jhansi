...
func TestMarkIdle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if err == nil {
					t.Errorf("MarkIdle from %s: want error, got nil", tt.from)
				}
				if events := s.DrainEvents(); len(events) != 1 || events[0].Name != "sandbox.idle_rejected" {
					t.Errorf("events = %v, want one sandbox.idle_rejected", events)
				}
				return
			}
			if err != nil {
				t.Errorf("MarkIdle from %s: unexpected erorr %v", tt.from, err)
			}
			if s.Status != SandboxReady {
				t.Errorf("Status = %s, want READY", s.Status)
			}
			if events := s.DrainEvents(); len(events) != 1 || events[0].Name != "sandbox.idle" {
				t.Errorf("events = %v, want one sandbox.idle", events)
			}
		})
	}
}
